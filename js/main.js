...
if(document.addEventListener) {
	...
	suspend.getElementsByTagName('button')[1].addEventListener('click', again, true);
} else if(document.attachEvent) {
	main.attachEvent('onmousemove', move);
	selfplane.imagenode.attachEvent('onclick', zanting);
	body.attachEvent('onmousemove', border);
	suspend.getElementsByTagName('button')[0].attachEvent('onclick', zanting);
	suspend.getElementsByTagName('button')[1].attachEvent('onclick', again);
}
...
function begin() {
	...
	if(mark == 20) {
		...
		//中飞机
		if(mark1 % 5 == 0) {
			enemys.push(new enemy(6, 30, 290, 46, 60, 50, 240, random(1, 3), 'img/中飞机爆炸.gif', 'img/enemy3_fly_1.png'))
		}
		//大飞机
		if(mark1 % 20 == 0) {
			enemys.push(new enemy(12, 60, 260, 110, 164, 100, 360, 1, 'img/大飞机爆炸.gif', 'img/enemy2_fly_1.png'));
		} else {
			enemys.push(new enemy(1, 10, 300, 34, 24, 10, 120, random(1, 6), 'img/小飞机爆炸.gif', 'img/enemy1_fly_1.png'));
		}
		mark = 0;
	}
